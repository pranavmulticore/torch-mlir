...
LogicalResult ConvertAtenOp<AtenGeluBackwardOp>::matchAndRewrite(
  ...
  // Compute
  Value kBeta0 = rewriter.create<mhlo::MulOp>(loc, inputTy, kAlpha, cstAlpha0);
  Value kBeta = rewriter.create<mhlo::MulOp>(loc, inputTy, kBeta0, half);
  Value erfArg =
      rewriter.create<mhlo::MulOp>(loc, inputTy, kAlpha, adaptor.getSelf());
  Value erf = rewriter.create<mlir::chlo::ErfOp>(loc, inputTy, erfArg);
  Value erfAdd = rewriter.create<mhlo::AddOp>(loc, inputTy, erf, one);
  Value cdf = rewriter.create<mhlo::MulOp>(loc, inputTy, erfAdd, half);
  Value inputSquared = rewriter.create<mhlo::MulOp>(
      loc, inputTy, adaptor.getSelf(), adaptor.getSelf());
  Value negHalfInputSquared =
      rewriter.create<mhlo::MulOp>(loc, inputTy, inputSquared, negHalf);
  Value expRes =
      rewriter.create<mhlo::ExpOp>(loc, inputTy, negHalfInputSquared);
  Value pdf = rewriter.create<mhlo::MulOp>(loc, inputTy, kBeta, expRes);
  Value pdfTimesInput =
      rewriter.create<mhlo::MulOp>(loc, inputTy, pdf, adaptor.getSelf());
  Value pdfTimesInputAddCdf =
      rewriter.create<mhlo::AddOp>(loc, inputTy, pdfTimesInput, cdf);
  rewriter.replaceOpWithNewOp<mhlo::MulOp>(op, inputTy, adaptor.getGradOutput(),
                                           pdfTimesInputAddCdf);
  return success();
}
